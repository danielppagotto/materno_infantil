...
servicos19_32 <- 
  vroom::vroom("~/GitHub/materno_infantil/02_script/04_servicos/servicos19_32.csv") |> 
  select(-`...1`)
...
melhor_modelo <- 
  read.csv("~/GitHub/materno_infantil/02_script/03_outputs_projecoes/prophet/metricas_previsoes_prophet/melhor_modelo.csv") |> 
  mutate(cod_regsaud = 
           as.character(cod_regsaud)) |> 
  mutate(reg_concat = 
                      paste0(cod_regsaud, 
