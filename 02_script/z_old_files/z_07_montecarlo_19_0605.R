...
executar_monte_carlo_desagregado <- function(n_sim = 10000, 
                                             ...
                                             participacao_medico_min = 0.5,
                                             participacao_medico_max = 1,
                                             servicos, 
                                             cobertura, 
                                             oferta, 
                                             foco_clinico){
  ...
  # # ]
  # 
  # if("percentual_cobertura" %in% colnames(sensibilidade)) {
  #   idx <- setdiff(1:ncol(sensibilidade), which(colnames(sensibilidade) == "percentual_cobertura"))
  #   valores <- abs(sensibilidade["percentual_cobertura", idx])
  #   parametro_maior_impacto <- colnames(sensibilidade)[idx[which.max(valores)]]
  # } else {
  #   parametro_maior_impacto <- "acoes_educacionais"  # valor padrão
  # }
  # 
  # 
  # plot_sensibilidade <- 
...
analisar_sensibilidade_geral <- function(resultado_mc) {
  ...
  plot_cor <- ggplot(cor_df, aes(x = reorder(Parametro, 
                                             abs(Correlacao)), 
...
plotar_comparacao_oferta_necessidade <- 
  function(resultado_mc, 
           cod_regsaude_alvo) {
...
plotar_distribuicao_regiao <- function(resultado_mc, 
                                       cod_regsaude_alvo) {
