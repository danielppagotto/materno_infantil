...
projecao <- function(regiao){
  ...
  write.csv(tabela_precisao, 
            file = paste0("~/GitHub/materno_infantil/02_script/03_outputs_projecoes/prophet/metricas_previsoes_prophet/metricas_", 
...
melhor_previsao <- serie_completa |> 
    filter(.model_desc == "Dados Históricos" |
           .model_desc == melhor_modelo$.model_desc)


write.csv(melhor_previsao, 
          file = paste0("~/GitHub/materno_infantil/02_script/03_outputs_projecoes/prophet/melhor_previsao_prophet/melhor_previsao_", 
                        unique(serie_completa$cod_regsaude), ".csv"),
          row.names = FALSE)
  
write.csv(serie_completa, 
            file = paste0("~/GitHub/materno_infantil/02_script/03_outputs_projecoes/prophet/data_previsoes/data_", 
                          unique(serie_completa$cod_regsaude), ".csv"),
            row.names = FALSE)
  
ggsave(filename = paste0("~/GitHub/materno_infantil/02_script/03_outputs_projecoes/prophet/plot_mensal_prophet/plot_mensal",  
...
tendencia_filtrada <- 
  ...
  grafico_tendencia <- 
    ...
    scale_x_continuous(breaks = seq(2000, 2032, by = 5))
  
  grafico_tendencia
  
  ggsave(filename = paste0("~/GitHub/materno_infantil/02_script/03_outputs_projecoes/prophet/plot_anual_prophet/plot_anual_",
                           unique(serie_completa$cod_regsaude), ".jpeg"),
